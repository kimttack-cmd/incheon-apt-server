...
async function fetchSale(serviceKey, lawdCd, dealYmd) {
  ...
  try {
    ...
    return arr.map(d => ({
      type: '매매',
      아파트: d['aptNm']?.trim() || d['아파트']?.trim() || '',
      법정동: d['umdNm']?.trim() || d['법정동']?.trim() || '',
      지번: d['jibun']?.trim() || d['지번']?.trim() || '',
      전용면적: parseFloat(d['excluUseAr'] || d['전용면적']) || 0,
      거래금액: parseInt((d['dealAmount'] || d['거래금액'] || '0').replace(/,/g, '')) || 0,
      층: parseInt(d['floor'] || d['층']) || 0,
      건축년도: parseInt(d['buildYear'] || d['건축년도']) || 0,
      년: parseInt(d['dealYear'] || d['년']) || 0,
      월: parseInt(d['dealMonth'] || d['월']) || 0,
      일: parseInt(d['dealDay'] || d['일']) || 0,
      거래일: `${d['dealYear']||d['년']}-${String(d['dealMonth']||d['월']).padStart(2,'0')}-${String(d['dealDay']||d['일']).padStart(2,'0')}`,
      등록일: d['rgstDate']?.trim() || d['등록일']?.trim() || '',
      해제여부: d['cdealType']?.trim() || d['해제여부']?.trim() || '',
    }));
  } catch(e) {
...
async function fetchRent(serviceKey, lawdCd, dealYmd) {
  ...
  try {
    ...
    const arr = Array.isArray(items) ? items : [items];
    return arr.map(d => ({
      type: parseInt(d['monthlyRent'] || d['월세금액']) > 0 ? '월세' : '전세',
      아파트: d['aptNm']?.trim() || d['아파트']?.trim() || '',
      법정동: d['umdNm']?.trim() || d['법정동']?.trim() || '',
      지번: d['jibun']?.trim() || d['지번']?.trim() || '',
      전용면적: parseFloat(d['excluUseAr'] || d['전용면적']) || 0,
      보증금액: parseInt((d['deposit'] || d['보증금액'] || '0').replace(/,/g, '')) || 0,
      월세금액: parseInt((d['monthlyRent'] || d['월세금액'] || '0').replace(/,/g, '')) || 0,
      층: parseInt(d['floor'] || d['층']) || 0,
      건축년도: parseInt(d['buildYear'] || d['건축년도']) || 0,
      년: parseInt(d['dealYear'] || d['년']) || 0,
      월: parseInt(d['dealMonth'] || d['월']) || 0,
